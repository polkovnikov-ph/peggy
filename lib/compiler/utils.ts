export const hex = (ch: string) => ch.charCodeAt(0).toString(16).toUpperCase();
 ...
 */
export const stringEscape = (s: string): string => s
  .replace(/\\/g,   "\\\\")   // Backslash
 ...
 * For portability, we also escape all control and non-ASCII characters.
 */
export const regexpClassEscape = (s: string): string => s
  .replace(/\\/g,   "\\\\")   // Backslash
 ...
 */
export const base64 = (u8: Uint8Array): string => {
...
/**
 * `Object.entries` with correct type.
 */
export const entries = Object.entries as <T,>(t: T) => ({ [K in keyof T]: [K, T[K]] })[keyof T][];